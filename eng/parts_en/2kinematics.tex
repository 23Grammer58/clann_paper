...
Let \(\vect{X}\) and \(\vect{x}\) denote point positions in the corresponding covariant basis vectors \(\vect{G}_{\alpha}\) and \(\vect{g}_{\alpha}\) in the reference (undeformed) \(\Omega_0 \subset \mathbb{R}^2\) and current (deformed) \(\Omega_t \subset \mathbb{R}^2\) configurations of the membrane midsurface, respectively.  
The deformation is defined by the one-to-one mapping \(\vect{x} = \vect{x}(\vect{X})\).
The surface deformation gradient in Einstein summation notation (used throughout this work) is \(\mathbb{F} = \vect{g}_{\alpha}\otimes\vect{G}^{\alpha}\), where $\vect{G}^{\alpha}$ are the contravariant basis vectors,
...
For an initially flat membrane, the contravariant basis vectors $\vect{G}^{\alpha}$ form an orthonormal basis.
To define the stretch measure we use the Laplace measure \(\vect{\xi} = (\xi_1 , \xi_2 , \xi_3)^T\) \cite{xi2023},
...
In two dimensions we introduce \textbf{Laplace stretch measure}
\begin{equation}
...
\end{equation}
In this case, the hyperelastic stretch energy is a function of the Laplace stretch, \(\psi = \psi(\vect{\xi})\).
