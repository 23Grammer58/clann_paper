...
the stretch energy \(\psi(\vect{\xi})\) with the Laplace stretch measure is approximated
...
\end{equation}
In our setting we do not distinguish parameters $y$ and consider only the stretch variables
$\boldsymbol{\xi}\in\mathbb{R}^3$, so that the stretch energy $\psi:\mathbb{R}^3\to\mathbb{R}$ is convex if for all
...
Let $\boldsymbol{\xi}\in\mathbb{R}^3$ be the input, $\vect z^{(0)}=\boldsymbol{0}$ be the initial hidden state, and for
layers $\ell=0,\dots,L-1$ define
\begin{equation}
  \vect z^{(\ell+1)} =
  \varphi\!\big(\mathbf{W}_z^{(\ell)} \vect z^{(\ell)} + \mathbf{W}_x^{(\ell)} \boldsymbol{\xi} + \mathbf{b}^{(\ell)}\big),
\end{equation}
where $\vect z^{(\ell)}\in\mathbb{R}^{h_\ell}$, $\mathbf{W}_z^{(\ell)}\in\mathbb{R}^{h_{\ell+1}\times h_\ell}$,
...
(ii) nonnegative weights on the $\vect z\!\to\!\vect z$ connections, $\mathbf{W}_z^{(\ell)}\!\ge\!0$ for all $\ell$
...
output). Under these assumptions the network output $\psi(\boldsymbol{\xi})$ is a convex function of the stretch
...
\begin{equation}
  ...
  \vect z = \varphi_{\beta}^2(s),\qquad
  \tilde{\psi} = \mathbf{W}_2^{\top} \vect z + b_2,\qquad \mathbf{W}_2 \ge 0.
...
\begin{equation}
  \vect z_0 = \varphi_{\beta}^2(\mathbf{b}_1),\qquad
  \psi(\boldsymbol{\xi}) = \mathbf{W}_2^{\top}\big(\vect z - \vect z_0\big),\qquad (b_2 \equiv 0).
  \label{eq:center_psi}
\end{equation}
Then $\psi(\mathbf{0})=0$. Since $\vect z_0$ does not depend on $\boldsymbol{\xi}$, 
...
and by the chain rule \eqref{eq:chain-rule} we obtain $\mathbb{S}(\mathbb{I})=\mathbf{0}$. 
...
\textbf{Gradient of the stretch energy}
...
Compared to the case with $\vect z=\varphi_\beta(\vect s)$, an additional factor $2\,\varphi_\beta(\vect s)$ appears, 
so that the contribution of each hidden neuron to the gradient is scaled by the activation magnitude.

\textbf{Hessian of the stretch energy}
...
\textbf{strict non-negativity and coercivity of the energy} are provided by the architectural calibration 
 $\psi_{\mathrm{phys}}(\boldsymbol{\xi}) = \mathbf{W}_2^{\top}(\vect z - \vect z_0) - \mathbf{r}_0^{\top}\boldsymbol{\xi}$,
...
nonnegative output weights, and centering of the stretch energy $\psi$ and the response $\vect r$.
