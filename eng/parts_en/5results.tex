...
At each step, $\mathbb{C}, \mathbb{S}$ were extracted for all triangles belonging to the selected observation window $w$, and because we use linear (P1) elements the tensors are constant within each triangle. 
Each loading scenario is encoded by a protocol index $p\in\{1,\dots,9\}$ that specifies the tuple $(w_1,\dots,w_4)$ according to Table~\ref{tab:test_protocols}, enabling systematic sampling of equibiaxial and non-equibiaxial load paths.
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    ...
    \end{tabular}
    \vspace{0.9em}
\captionof{table}{Biaxial testing protocols}
...
The coefficient of determination is computed separately for each tensor component $\alpha\in\{xx,yy,xy\}$:
\begin{equation}\label{eq:r_squared}
  R^2_{\alpha} = 1 - \frac{\sum_{i=1}^N (S_{\alpha,i} - \hat{S}_{\alpha,i})^2}{\sum_{i=1}^N (S_{\alpha,i} - \bar{S}_{\alpha})^2},
  \qquad \alpha\in\{xx,yy,xy\},
\end{equation}
where $S_{\alpha,i}$ are experimental values of component $\alpha$, $\hat{S}_{\alpha,i}$ are model predictions, $\bar{S}_{\alpha}$ is the mean value of component $\alpha$, and $N$ is the number of data points.

\textbf{Pointwise relative error.}
\begin{equation}\label{eq:rel_error}
  \epsilon = \frac{\| \hat{\mathbb S} - \mathbb S \|_{L2}}{\| \mathbb S \|_{L2}}.
\end{equation}
...
\begin{equation}\label{eq:p1_error}
  \epsilon_{\mathrm{P1}} = \frac{\|\hat{ \mathbb S_{12} }- \mathbb S_{12} \|_{L2}}{s_0 + \| \mathbb S_{12} \|_{L2}},\qquad s_0 = \max(\hat{S_{12}}).
\end{equation}

\textbf{Absolute integral error for stresses (Frobenius norm).}
\begin{equation}\label{eq:l2_abs_stress_cell}
  \|e\|_{L^2} = \Bigg( \sum_{T} \| \mathbb S_{T} - \hat{\mathbb S_{T}} \|_F^{2}\, |T| \Bigg)^{\tfrac12}.
\end{equation}
...
\begin{equation}\label{eq:l2_rel_stress}
  \|e\|_{L^2,\,\mathrm{rel}}\;=\; \frac{\Big( \sum\limits_{T} 
  \| \mathbb S_{T} - \hat{\mathbb S_{T}} \|_F^{2}\, |T| \Big)^{\tfrac12}}
  {\Big( \sum\limits_{T} \| \mathbb S_{T} \|_F^{2}\, |T| \Big)^{\tfrac12}}\,.
\end{equation}
...
\begin{figure}[H]
  \centering
\includegraphics[width=0.5\textwidth]{../img/loss_curve.png}
...
\subsection{Interpolation and extrapolation of loading curves}
  ...
  For quality assessment, we used the coefficient of determination $R^2$.
  ...
  \begin{figure}[H]
    ...
    \caption{Interpolation of the equibiaxial loading curve from three anchor points ($n=\{1,5,10\}$)}
  ...
  \begin{figure}[H]
    ...
    \caption{Extrapolation: prediction of the $p = 9$ loading curve using a model trained on the $p = 1$}
  ...
  \begin{figure}[H]
    ...
    \caption{Predicted loading curves for window $w = \text{2-element}$ for protocols $p = 1..9$}
    \label{fig:curves_2}
  \end{figure}
...
\subsection{Computational efficiency comparison}
  ...
  All computations were performed on a laptop with an AMD Ryzen 7 6800H processor.
