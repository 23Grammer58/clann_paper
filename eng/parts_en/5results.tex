...
\begin{equation}\label{eq:r_squared}
  R^2 = 1 - \frac{\sum_{i=1}^N (\mathbb{S}_i - \hat{\mathbb{S}}_i)^2}{\sum_{i=1}^N (\mathbb{S}_i - \bar{\mathbb{S}})^2},
  \qquad R^2_{\alpha}(D_{\mathrm{val}}),\quad \alpha\in\{xx,yy,xy\}.
\end{equation}
where $\mathbb{S}_i$ are experimental values, $\hat{\mathbb{S}}_i$ are model predictions, $\bar{\mathbb{S}}$ is the mean, and $N$ is the number of points.
