...
% Аффинная проекция
\node[neur0, right=14mm of x1, yshift=12mm] (h11) {\scriptsize $z_1$};
\node[neur0, right=14mm of x1] (h12) {\scriptsize $z_2$};
\node[neur0, right=14mm of x1, yshift=-12mm] (h13) {\scriptsize $z_3$};
\node[align=center, right=14mm of x1, yshift=-22mm] (vdots0) {$\vdots$};
\node[neur0, right=14mm of x1, yshift=-34mm] (h14) {\scriptsize $z_h$};
\draw[->] (x1) -- (h11.west);
\draw[->] (x2) -- (h11.west);
\draw[->] (x3) -- (h11.west);
\draw[->] (x1) -- (h12.west);
...
\node[actblk, above=2mm of h11] (soft) {$z_i = \mathrm{softplus^2}(\beta (\vect W_1\,\vect \xi + b_1))/\beta^2, \; \; i = 1..h$};
...
\node[readblk, right=22mm of h12, yshift=-11mm] (readout) {$\psi(\vect{\xi}) = \vect{W}_2^{\top}\big(\vect z - \vect z_0\big) - \vect{r}_0^{\top}\vect{\xi}$};
...
\node[gradblk, below=8mm of readout] (Sblock) {$\mathbb{S} = \frac{\partial \psi}{\partial \vect{\xi}} \cdot \frac{\partial \vect{\xi}}{\partial \mathbb{C}}$};
