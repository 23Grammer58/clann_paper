...
\node[cblk, left=14mm of x2] (C) {$\mathbf{C}$};
\draw[->] (C.east) -- (x1.west);
\draw[->] (C.east) -- (x2.west);
\draw[->] (C.east) -- (x3.west);
...
\node[readblk, right=22mm of h12, yshift=-11mm] (readout) {$\psi(\boldsymbol{\xi}) = \mathbf{W}_2^{\top}\big(z - z_0\big) - \mathbf{r}_0^{\top}\boldsymbol{\xi}$};
...
\node[gradblk, below=8mm of readout] (Sblock) {$\mathbf{S} = \partial\,\psi/\partial\,\boldsymbol{\xi} \cdot \partial\,\boldsymbol{\xi}/\partial\,\boldsymbol{\vect C}$};
